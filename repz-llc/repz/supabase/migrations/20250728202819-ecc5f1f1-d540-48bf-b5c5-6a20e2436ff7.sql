-- Phase 1: Database Cleanup and Standardization (Fixed)
...
SELECT 
  ...
  up.phone,
  'client' as role,
  cp.subscription_tier::text as subscription_tier,
  cp.tier_features,
  cp.age_years,
  cp.sex::text as sex,
  cp.primary_goal::text as primary_goal,
  cp.created_at,
  cp.updated_at
...
SELECT 
  ...
  up.phone,
  'coach' as role,
  'longevity' as subscription_tier, -- Coaches get highest tier access
...
ADD CONSTRAINT IF NOT EXISTS client_profiles_age_check CHECK (age_years IS NULL OR (age_years >= 13 AND age_years <= 120));

ALTER TABLE client_profiles 
ADD CONSTRAINT IF NOT EXISTS client_profiles_training_days_check CHECK (training_days_per_week IS NULL OR (training_days_per_week >= 1 AND training_days_per_week <= 7));
