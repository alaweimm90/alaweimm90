...
class AgentValidator {
    ...
    validateAgentRegistration() {
        ...
        const registryPath = path.join(__dirname, '..', '..', 'tools', 'atlas', 'agents', 'registry.ts');
        ...
        const hasAgentInterface = registryContent.includes('interface Agent') || registryContent.includes('export interface Agent');
        const hasAgentRegistry = registryContent.includes('agentRegistry') || registryContent.includes('AgentRegistry');

        this.validationResults.push({
            test: 'Agent Registry Structure',
            status: hasAgentInterface && hasAgentRegistry ? 'PASS' : 'WARN',
            details: hasAgentInterface && hasAgentRegistry ?
    ...
    validateCapabilityMapping() {
        ...
        const requiredCapabilities = [
            'analysis', 'complexity', 'chaos', 'refactor', 'optimize',
        ...
        this.validationResults.push({
            ...
            details: missingCapabilities.length === 0 ?
    ...
    validateTaskRouting() {
        ...
        const routerPath = path.join(__dirname, '..', '..', 'tools', 'atlas', 'orchestration', 'router.ts');
        ...
        const hasRoutingLogic = routerContent.includes('RoutingDecision') || routerContent.includes('route');

        this.validationResults.push({
            test: 'Task Routing Logic',
            status: hasRoutingLogic ? 'PASS' : 'WARN',
            details: hasRoutingLogic ?
                'Basic routing structures found' :
    ...
    validateLoadBalancing() {
        ...
        // Check load balancer
        const lbPath = path.join(__dirname, '..', '..', 'tools', 'atlas', 'orchestration', 'load-balancer.ts');
        ...
        const balanced = Array.from(agentLoads.values()).every(load =>
            Math.abs(load - avgLoad) < 20
        );
        ...
        this.validationResults.push({
            ...
            details: balanced ?
                'Load well balanced across agents' :
    ...
    generateReport() {
        ...
        if (report.summary.failed > 0) {
            console.log('\nFAILED TESTS:');
            report.results.filter(t => t.status === 'FAIL').forEach(test => {
