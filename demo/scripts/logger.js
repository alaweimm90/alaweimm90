...
class AtlasLogger {
    ...
    endOperation(operationName, success = true, result = {}) {
        const operation = this.metrics.operations.find(op =>
            op.name === operationName && op.status === 'running'
        );
    ...
    finalize() {
        ...
        this.metrics.summary = {
            ...
            successfulOperations: this.metrics.operations.filter(op => op.status === 'completed').length,
