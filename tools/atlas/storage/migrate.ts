/**
 * ATLAS Storage Migration Utility
...
export async function migrateStorage(options: MigrationOptions): Promise<MigrationResult> {
  ...
  try {
    ...
    const source =
      options.source === 'json'
        ? new JsonStorageBackend(options.sourcePath || '.atlas/data')
        : new SqliteStorageBackend(options.sourcePath || '.atlas/atlas.db');

    // Create target backend
    log(`Creating target backend: ${options.target}`);
    const target =
      options.target === 'json'
        ? new JsonStorageBackend(options.targetPath || '.atlas/data-migrated')
        : new SqliteStorageBackend(options.targetPath || '.atlas/atlas-migrated.db');
...
Usage:
  npx tsx tools/atlas/storage/migrate.ts <source> <target> [options]
...
Examples:
  ...
  npx tsx tools/atlas/storage/migrate.ts json sqlite --verbose

  # Migrate specific collection
  npx tsx tools/atlas/storage/migrate.ts json sqlite --collection agents --verbose

  # Dry run to see what would be migrated
  npx tsx tools/atlas/storage/migrate.ts json sqlite --dry-run --verbose
`);
    process.exit(0);
  }
