...
export function registerCommands(context: vscode.ExtensionContext, atlasAPI: AtlasAPI | undefined) {
    // Configure API Key
    const configureApiKeyCommand = vscode.commands.registerCommand('atlas.configureApiKey', async () => {
        ...
        if (apiKey) {
            await vscode.workspace.getConfiguration('atlas').update('apiKey', apiKey, vscode.ConfigurationTarget.Global);
        ...
        try {
            vscode.window.withProgress({
                ...
                if (taskResult.status === 'completed' && taskResult.result) {
                    ...
                    vscode.window.showErrorMessage(`ATLAS: Analysis failed - ${taskResult.error || 'Unknown error'}`);
                }
            });
        } catch (error) {
        ...
        try {
            vscode.window.withProgress({
                ...
                if (taskResult.status === 'completed' && taskResult.result) {
                    ...
                    vscode.window.showErrorMessage(`ATLAS: Code generation failed - ${taskResult.error || 'Unknown error'}`);
                }
            });
        } catch (error) {
        ...
        try {
            vscode.window.withProgress({
                ...
                if (taskResult.status === 'completed' && taskResult.result) {
                    ...
                    vscode.window.showErrorMessage(`ATLAS: Code review failed - ${taskResult.error || 'Unknown error'}`);
                }
            });
        } catch (error) {
            ...
            vscode.window.withProgress({
                ...
                if (taskResult.status === 'completed' && taskResult.result) {
                    ...
                    const replace = await vscode.window.showInformationMessage(
                        ...
                        'Replace Code', 'Copy to Clipboard'
                    );
                    ...
                    vscode.window.showErrorMessage(`ATLAS: Refactoring failed - ${taskResult.error || 'Unknown error'}`);
                }
            });
        } catch (error) {
