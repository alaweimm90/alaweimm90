...
class AtlasTasksProvider implements vscode.TreeDataProvider<TaskItem> {
    ...
    readonly onDidChangeTreeData: vscode.Event<TaskItem | undefined | null | void> = this._onDidChangeTreeData.fire;
    ...
    async getChildren(element?: TaskItem): Promise<TaskItem[]> {
        if (!this.atlasAPI) {
            return [new TaskItem('Configure API Key', 'Please configure your ATLAS API key', vscode.TreeItemCollapsibleState.None, 'configure')];
        }
        ...
        if (!element) {
            ...
            try {
                ...
                if (runningTasks.length > 0) {
                    items.push(new TaskItem(`Running (${runningTasks.length})`, '', vscode.TreeItemCollapsibleState.Expanded, 'category', runningTasks));
                }
                if (queuedTasks.length > 0) {
                    items.push(new TaskItem(`Queued (${queuedTasks.length})`, '', vscode.TreeItemCollapsibleState.Collapsed, 'category', queuedTasks));
                }
                if (completedTasks.length > 0) {
                    items.push(new TaskItem(`Completed (${completedTasks.length})`, '', vscode.TreeItemCollapsibleState.Collapsed, 'category', completedTasks));
                }
                if (failedTasks.length > 0) {
                    items.push(new TaskItem(`Failed (${failedTasks.length})`, '', vscode.TreeItemCollapsibleState.Collapsed, 'category', failedTasks));
                }

                return items;
            } catch (error) {
                return [new TaskItem('Error loading tasks', error.message, vscode.TreeItemCollapsibleState.None, 'error')];
            }
        } else if (element.type === 'category') {
            // Show tasks in this category
            return element.tasks.map(task => new TaskItem(
...
class AtlasAgentsProvider implements vscode.TreeDataProvider<AgentItem> {
    ...
    async getChildren(element?: AgentItem): Promise<AgentItem[]> {
        if (!this.atlasAPI) {
            return [new AgentItem('Configure API Key', 'Please configure your ATLAS API key', vscode.TreeItemCollapsibleState.None, 'configure')];
        }
        ...
        if (!element) {
            ...
            try {
                ...
                return [new AgentItem('Error loading agents', error.message, vscode.TreeItemCollapsibleState.None, 'error')];
            }
        } else if (element.type === 'provider') {
            // Show agents for this provider
            return element.agents.map(agent => {
                const icon = agent.health.status === 'healthy' ? '✓' : agent.health.status === 'degraded' ? '⚠' : '✗';
                return new AgentItem(
                    `${icon} ${agent.name}`,
...
export function registerTreeViews(context: vscode.ExtensionContext, atlasAPI: AtlasAPI | undefined) {
    ...
    const showTaskDetailsCommand = vscode.commands.registerCommand('atlas.showTaskDetails', (task: AtlasTask) => {
        ...
        if (task.result) {
            ...
            outputChannel.appendLine(typeof task.result === 'string' ? task.result : JSON.stringify(task.result, null, 2));
        }
        if (task.error) {
            outputChannel.appendLine('');
    ...
    const showAgentDetailsCommand = vscode.commands.registerCommand('atlas.showAgentDetails', (agent: AtlasAgent) => {
        ...
        outputChannel.appendLine(`  Success Rate: ${(agent.performance.success_rate * 100).toFixed(1)}%`);
        outputChannel.appendLine(`  Quality Score: ${agent.performance.avg_quality_score.toFixed(1)}`);
