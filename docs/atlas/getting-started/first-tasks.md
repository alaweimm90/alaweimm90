# First Tasks with ATLAS

Learn how to perform your first tasks with ATLAS, from registering agents to submitting various types of development tasks.
...
# View agent details
atlas agent show claude-sonnet-4
```
...
atlas task list --status completed --limit 5
...
atlas analyze file src/utils/helpers.js --metrics complexity
...
atlas task result task_abc123 --format detailed

# Export results to file
atlas task result task_abc123 --output result.json

# View results in browser (if available)
atlas task result task_abc123 --web
```
...
atlas task apply task_abc123 --file-path src/new-feature.js

# Create a new file with results
atlas task apply task_abc123 --create-file src/components/NewComponent.tsx

# Apply with backup
atlas task apply task_abc123 --file-path src/api.js --backup
```
...
atlas task result task_failed_123

# Retry with same agent
atlas task retry task_failed_123

# Retry with different agent
atlas task retry task_failed_123 --agent gpt-4-turbo

# Force retry ignoring previous failures
atlas task retry task_failed_123 --force
```
...
atlas task submit --type code_generation --description "Part 2 of user management" --max-tokens 1000
```
...
2. **[Multi-Agent Orchestration](../architecture/components.md#task-router)** - Understand how ATLAS selects agents
3. **[Repository Analysis](../cli/analysis.md)** - Analyze entire codebases
4. **[Integration Guides](../integration/)** - Integrate ATLAS into your workflow

---

## Getting Help

- **Task Status**: `atlas task status <task-id>`
- **Agent Health**: `atlas agent health <agent-id>`
- **System Logs**: `atlas logs --tail`
